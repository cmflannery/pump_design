...
Centrifugal pump design tool. As a reference value, an ethanol / liquid oxygen engine is assumed.
It corresponds only to a single stage centrifugal pump. This tool can only be used for approximation.
Cf. Fundamentals of pump design Yutaka Takeda

Centrifugal Pump design tool with EA/LOX engine
Only use for Single-stage certrifugal pump.
...
class Rocket(object):
    ...
    def __init__(self, thrust, Isp, OFratio):
        ...
        self.density_LNG = 485.3 # kg/m3 (LNG)
    ...
    def culc(self):
        ...
        self.flow_fuel = self.mdot_fuel / self.density_LNG
    ...
    def show(self):
        ...
        print("Propellant Mass Flow:\t%.2f (kg/s)" % (self.mdot_proppelant))
        print("Oxidizer Mass Flow:\t%.2f (kg/s)" % (self.mdot_oxidant))
        print("Oxidizer Volumetric Flow Rate :\t%.4f (m3/s)" % (self.flow_oxidant))
...
class Pump(object):
    ...
    def show(self):
        ...
        print("Action fluid density:\t%d (kg/m3)" % (self.density))
        print("Rotational Speed N:\t%d (rpm)" % (self.rpm))
        print("Rotational Frequency N:\t%.1f (rad/s)" % (self.omega))
        print("Pump discharge pressure:\t%.1f (MPa)" % (self.press_out))
        print("Mass Flow Q:\t%.2f (m3/min)" % (self.flow_Q))
        print("Mass Flow q:\t%.2f (m3/sec)" % (self.flow_q))
        print("Total Head H:\t%.2f (m)" % (self.head_total))
        print("比速度 *Something* Speed Ns:\t%.1f (m min)" % (self.Ns))
...
#        print("Hsv from(18):\t%.2f (m)" % (self.head_sv2))
        print("Shroud diameter Ds:\t%.2f (mm)" % (self.Ds * 1000))
        print("Hub Diameter Dh:\t%.2f (mm)" % (self.Dh * 1000))

        print("---- Impeller outlet design ----")
        print("sqrt(D2/B2):\t%.2f" % (self.sqrtD2B2))
        print("Theoretical than speed Ns_th:\t%.2f" % (self.Ns_th))
        print("Slip coefficient sigma:\t%.2f" % (self.sigma))
        print("Parameter chi:\t%.2f" % (self.chi))
        print("Outside diameter D2:\t%.2f (mm)" % (self.D2 * 1000))
        ...
        print("Volute Height T:\t%.2f (mm)" % (self.T * 1000))
        print("Hth(Impeller):\t%.2f (m)" % (self.Hth_impeller))
        print("Hth(Volute):\t%.2f (m)" % (self.Hth_volute))
        print("Q at maximum efficiency:\t%.2f (m3/min)" % (self.flow_Q_max))
...
def main():
    plt.close('all')
    thrust = 4448.22  # Thrust in N (1000 lbf)
    Isp = 260  # reasonable value for Isp
    Of_ratio = 2.77

    rocket = Rocket(thrust, Isp, Of_ratio)
    rocket.culc()
    rocket.show()
